...
public:
	...
	int mMouseWheel;
	unsigned mTimeBias; //timeGetTimeにはかせるゲ?
	unsigned mThreadId;
...
// グロ?バル変数:
HINSTANCE hInst;								// 現在のイン??フェイス
TCHAR szTitle[MAX_LOADSTRING];					// ?イトル バ?のテキスト
TCHAR szWindowClass[MAX_LOADSTRING];			// メイン ウィンドウ クラス名

// このコ?ド モジュ?ルに含まれる関数の宣言を?送します:
                     ...
                     int       nCmdShow)
{
	...
	///起動前の例外は継続不可?なので、メッセ?ジ?ックスを出して終わらせる
	try{
		...
		MessageBoxA( 
			windowHandle,
			"치명적인 문제가 있어 계속할 수 없습니다.죄송합니다만 종료하겠습니다.",
			"치명적오류?",
			MB_OK | MB_ICONERROR );
		wc.requestEnd(); //終われ。
		wc.update(); //中で終了処理
	...
	while ( true ){
		...
		if ( PeekMessage( &msg, NULL, 0, 0, PM_NOREMOVE ) ){
			if ( GetMessage( &msg, NULL, 0, 0 ) ){
				...
				break; //WM_QUITきました。ル?プ抜けます。
			}
		}
		//終わってればwindow破棄
		if ( gImpl->mEnded ){ //終わってなければ更新ル?プ
...
//    Windows 95 より前の Win32 システ?と互換させる場合にのみ必要です。
//    アプリケ?ションが、関連付けられた
//    正しい?式の小さいアイコンを取得できるようにするには、
//    この関数を呼び出してください。
//
ATOM MyRegisterClass(HINSTANCE hInstance)
{
...
//
//   目的: インス?ンス ハンドルを保存して、メイン ウィンドウを作成します。
//
//   コメント:
//
//        この関数で、グロ?バル変数でインス?ンス ハンドルを保存し、
//        メイン プログラ? ウィンドウを作成および?示します。
//
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	HWND hWnd;

	hInst = hInstance; // グロ?バル変数にインス?ンス処理を格?します。
//----
	DWORD style = 0;
	if ( gImpl->mFullScreen ){
		style = WS_POPUP;
...
//
//  目的:  メイン ウィンドウのメッセ?ジを処理します。
//
//  WM_COMMAND	- アプリケ?ション メニュ?の処理
//  WM_PAINT	- メイン ウィンドウの?画
//  WM_DESTROY	- 中?メッセ?ジを?示して戻る
//
//
