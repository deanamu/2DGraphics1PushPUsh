...
//先頭の0をそのままにして返す。
//何故?イナス判定を中でやらないかと言えば、unsignedで受け取れば
//?イナスのintが入らないし、intで受け取ればunsignedが入らないからである
...
int toString( char* out, float a, int precision ){
	...
	//e10が[-2,precision)にあればe記?は使わない。その方が短くなるからだ。
	if ( ( e10 >= -2 ) && ( e10 < precision ) ){ 
		if ( e10 >= 0 ){ //第一位が整数の場合
			for ( int i = 0; i < n; ++i ){
				*p++ = '0' + s[ i ];
				if ( ( i == e10 ) && ( i != n - 1 ) ){ //最後の数なら小数?無用
					*p++ = '.'; //小数?出力
				}
			}
			//eがでかい場合は?分の0が必要
