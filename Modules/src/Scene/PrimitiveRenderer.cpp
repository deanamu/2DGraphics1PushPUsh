...
public:
	...
	mBatchCapacity( batchCapacity + 1 ),//???用に+1
	mVertexPosition( 0 ),
	mBatchPosition( 0 ),
	mBatches( 0 ),
	mLockedVertexBuffer( 0 ){
		...
		mBatches[ 0 ].mPrimitiveNumber = 0x7fffffff; //0番は???バッ?
	...
	Batch* addBatch(){
		...
		mBatches[ mBatchPosition + 1 ] = mBatches[ mBatchPosition ]; //コピ?
		++mBatchPosition;
	...
	float depth ){
		...
		b->mPrimitiveNumber += 2; //三角?二つであることに注意
