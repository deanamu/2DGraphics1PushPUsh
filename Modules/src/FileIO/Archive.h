...
public:
	...
	mDummy( false ){
		if ( !name ){
			mDummy = true; //???ア?カイブ。直接フ?イルから読み込みます。
			return;
		}
		//フ?イルを開けてメンバに持っておく。
		setlocale( LC_ALL, "" ); //これがないと日?語フ?イル名を受け付けない
		...
		for ( int i = 0; i < mFileNumber; ++i ){
			...
			char* name = NEW char[ nameLength + 1 ]; //終?NULLで+1
			mStream->read( name, nameLength );
			name[ nameLength ] = '\0'; //終?NULL
			//mEntriesはmap< char*, Entry >
			mEntries.add( name, e ); //mapに格?
			//情報を吐き出してみよう。正しいか？
	...
	const char* name ){
		if ( mDummy ){
			...
			*streamOut = NEW ifstream( name, ifstream::binary );
			...
			if ( mFileNumber == 0 ){ //ないア?カイブなら常に見つからない
				*entryIndex = -1;
				*streamOut = 0;
			}else{
	...
	void allocate( char** dataOut, int size, int entryIndex ){
		if ( mDummy ){
			...
			*dataOut = NEW char[ size + 1 ]; //親切設計NULL終?。
		}else{
			STRONG_ASSERT( entryIndex >= 0 );
			const Entry& e = *mEntries.value( entryIndex );
			int allocSize = max( e.mNecessaryBufferSize, e.mOriginalSize + 1 ); //展開に必要なバッフ?サイズがあるならそれも書く。
	...
	ifstream* stream ){
		...
		int readOffset = 0;
		if ( compressed ){
			readOffset = necessaryBufferSize - size; //後ろにロ?ド。展開に?分に必要な場合があるのでoriginalSizeではない。
		}
		stream->read( data + readOffset, size );
		if ( stream->gcount() != size ){ //エラ?か
			*errorOut = true;
			return;
		}
		if ( compressed ){
