...
namespace GameLib {
	...
	namespace { //無名
		...
		public:
			...
			void start(void* windowHandle) {
				...
				//文字?画
				//アスキ?部分の?(アスキ?(126-32+1=95文字+63文字で158文字
			...
			void postUpdate() {
				...
				for (int y = 0; y < mHeight; ++y) {
					for (int x = 0; x < mWidth; ++x) {
						dst[x] = 0xff000000 | src[x]; //強制アルフ?ff
					}
					src += mWidth;
					dst += pitch / 4;
				}
			...
			//サンプルライブラリ用
			Array< unsigned int > mVideoMemoryWithPadding;
	...
	*/
	...
	void Framework::enableFullScreen( bool f ){
		if ( gImpl->mFullScreenForbidden ){ //禁?されている
			return;
		}
	...
	*/
	...
	void WindowCreator::update() {
		...
		//デバイスロストこいているようなら復帰を試みるが、基?的に寝る。
	/*
		if ( !gm.canRender() ){
			gm.restore();
			sleepFlag = true;
	...
	*/
		...
		//上流(???ンとか)から終了命令が来ているか?ェック
		if (isEndRequested()) { //来ていればFrameworkに終了命令を?達
			f.requestEnd();
		}
		if (sleepFlag) {
